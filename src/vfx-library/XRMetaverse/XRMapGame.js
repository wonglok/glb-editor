...
export class XRMapGame extends Object3D {
  constructor({ xrPlayer, api, params, onDone, onDoneMyAvatar }) {
    ...
    let setup = async () => {
      ...
      let cameraReal = await api.ready.camera
      ...
      api.onLoop(() => {
        if (renderer.xr.isPresenting) {
          xrPlayer.position.copy(player.position)
          // xrPlayer.position.y += 1
          // xrPlayer.position.z += 1
        } else {
          //
          cameraReal.position.copy(camera.position)
          cameraReal.quaternion.copy(camera.quaternion)
        }
      })
      state.player = player
...
XRMapGame.key = getID() + GameControls.key
